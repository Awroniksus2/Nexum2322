...
const { sendMedicalReport } = require('./services/email');
const {
  getDb,
...
app.post('/bot/webhook', async function(req, res) {
  try {
    ...
    if (text === '/start') {
      ...
      state[telegramId] = { step: 'awaiting_phone' };
      await fetch('https://api.telegram.org/bot' + (process.env.TELEGRAM_BOT_TOKEN || process.env.BOT_TOKEN) + '/sendMessage', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          chat_id: telegramId,
          text: '📱 Для верифікації натисніть кнопку нижче, щоб поділитися номером телефону:',
          parse_mode: 'Markdown',
          reply_markup: { keyboard: [[{ text: '📱 Поділитися номером телефону', request_contact: true }]], resize_keyboard: true, one_time_keyboard: true }
        })
      });
      ...
      const { name, city, hospital, specialty, phone } = state[telegramId];
      delete state[telegramId];
      const code = await generateUniqueCode();
      const password = await saveDoctor(telegramId, { name, city, hospital, specialty, email: text, code, phone: phone || '' });
      await fetch('https://api.telegram.org/bot' + (process.env.TELEGRAM_BOT_TOKEN || process.env.BOT_TOKEN) + '/sendMessage', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      state[telegramId] = { step: 'name', phone: existing.phone || '' };
...
// /api/debug/survey — ВИДАЛЕНО з продакшну
...
async function getDoctorSurvey(doctorCode, surveyKey) {
  try {
    ...
    if (!raw) {
      console.log('[getDoctorSurvey] no surveys for', doctorCode);
      return null;
    }
    ...
    if (Array.isArray(data) && data.length > 0) {
      console.log('[getDoctorSurvey] old array format → DIRECT mode');
    ...
    if (data && !Array.isArray(data) && Array.isArray(data.questions) && data.questions.length > 0) {
      const aiMode = data.aiMode === true;
      console.log('[getDoctorSurvey] flat object → aiMode:', aiMode);
      return { questions: data.questions, aiMode, surveyName: 'Survey', surveyKey: 'default' };
    }
    ...
    if (data && !Array.isArray(data) && typeof data === 'object' && !data.questions) {
      ...
      if (!entries.length) {
        console.log('[getDoctorSurvey] no valid survey entries');
        return null;
      }

      let chosen = surveyKey ? entries.find(([k]) => k === surveyKey) : null;

      if (!chosen && defaultKey) {
        chosen = entries.find(([k]) => k === defaultKey);
        if (chosen) console.log('[getDoctorSurvey] using _defaultKey:', defaultKey);
      }
      ...
      console.log('[getDoctorSurvey] fsId:', fsId,
        '| key:', chosenKey,
        '| name:', chosenSurvey.name,
        '| questions:', chosenSurvey.questions.length,
        '| aiMode:', aiMode,
        '| mode:', aiMode ? '🤖 AI (Groq)' : '✅ DIRECT (exact)');

      return {
        questions: chosenSurvey.questions,
        aiMode,
        surveyName: chosenSurvey.name,
        surveyKey: chosenKey
      };
    }
...
// ── POST /api/chat (з перевіркою doctorCode) ──────────────────
app.post('/api/chat', async function(req, res) {
  try {
    const { history, patientName, lang, doctorCode } = req.body;

    // Вимагаємо doctorCode — без нього чат не стартує
    if (!doctorCode) {
      return res.status(400).json({ error: 'doctorCode required' });
    }
    ...
    const hasCustomQuestions = surveyData &&
  Array.isArray(surveyData.questions) &&
  surveyData.questions.length > 0;
    
    console.log('[/api/chat] doctorCode:', doctorCode,
      '| surveyKey:', surveyKey,
      '| questions:', surveyData?.questions?.length || 0,
      '| aiMode:', surveyData?.aiMode,
      '| mode:', hasCustomQuestions ? '✅ DIRECT' : '🤖 AI');
 ...
 if (hasCustomQuestions) {
  ...
  if (answerCount >= total && surveyData.aiMode === true) {
    // Всі питання конструктора відповіді отримали — передаємо в AI для додаткових питань
 ...
 if (answerCount >= total && surveyData.aiMode !== true) {
  ...
  // ── НОВА ФУНКЦІЯ: витягує options і type для поточного питання ──
  ...
  try {
    ...
    try {
      ...
      if (surveyData && Array.isArray(surveyData.questions) && surveyData.questions.length > 0) {
        ...
        console.log('[submit] surveyResults:', answers.length, 'q for', surveyData.surveyName);
      }
    } catch(e) {
    ...
    if (dashId && surveyResults) {
      try {
        const { patientsCol } = require('./services/firebase');
        const docId = dashId.replace('fb_', '');
        const dc = doctorCode.trim().toUpperCase();
        const snap = await patientsCol(dc).doc(docId).get();
        const existing = (snap.data() || {}).surveyResults || [];
    ...
    // email відключено — дані зберігаються в медичному кабінеті
