...
function showOptions(options, isMulti) {
 ...
 wrap.style.cssText = 'display:flex;flex-wrap:wrap;gap:8px;justify-content:center;padding:10px 14px 8px;background:var(--surface);border-top:1px solid var(--border);';
  options.forEach(function(opt) {
    var btn = document.createElement('button');
    btn.textContent = opt; btn.dataset.opt = opt;
   btn.style.cssText = 'background:rgba(194,54,90,0.1);color:#2D1A22;border:1.5px solid rgba(194,54,90,0.3);border-radius:20px;padding:11px 22px;font-size:15px;font-family:inherit;cursor:pointer;transition:background .15s,border-color .15s,color .15s;';
    btn.onclick = function() {
      ...
      if (!isMulti) { hideOptions(); _injectAndSend(opt); }
        ...
        if (idx === -1) {
          ...
          btn.style.background = 'rgba(194,54,90,0.45)'; btn.style.borderColor = 'rgba(194,54,90,0.9)'; btn.style.color = '#fff';
        } else {
          _multiSelected.splice(idx, 1);
         btn.style.background = 'rgba(194,54,90,0.1)'; btn.style.borderColor = 'rgba(194,54,90,0.3)'; btn.style.color = '#2D1A22';
        }
      }
    };
  ...
  if (isMulti) {
    ...
    confirmBtn.style.cssText = 'background:#C2365A;color:#fff;border:none;border-radius:20px;padding:11px 22px;font-size:15px;font-weight:600;font-family:inherit;cursor:pointer;margin-left:6px;transition:opacity .15s;';
